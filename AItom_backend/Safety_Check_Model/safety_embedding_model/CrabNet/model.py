...
class Model():
    ...
    def train(self):
        ...
        _ = time() - ti
        _ = len(self.train_loader.dataset)
    ...
    def collect_embeddings(self, loader=None, capture_head=True,
                           capture_encoder=False, apply_pooling=True,
                           apply_gating=False):
        """
        Collect intermediate embeddings from the underlying CrabNet model.

        Parameters
        ----------
        loader : torch.utils.data.DataLoader, optional
            Loader to iterate over. Defaults to the most recently used
            validation loader (`self.data_loader`). If that is unavailable,
            the training loader is used.
        capture_encoder : bool, optional
            Whether to capture the shared composition encoder output.
        capture_head : bool, optional
            Whether to capture the property-specific head representation.
        apply_pooling : bool, optional
            If True (default), element-wise head outputs are masked, averaged
            over the valid elements, and flattened to `(batch, -1)` so that the
            returned tensor is directly usable as a per-formula embedding.
        apply_gating : bool, optional
            If True and `apply_pooling` is enabled, replicate CrabNet's final
            gating logic by splitting pooled features into value/logit chunks
            and applying `torch.sigmoid` to the logit portion. When enabled,
            an additional key `'head_logits'` is included in the returned
            dictionary containing the pooled logit tensor.

        Returns
        -------
        dict
            Dictionary containing tensors keyed by embedding name. Available
            keys are:
                - 'encoder': shape `(batch, max_atoms, d_model)`
                - 'head': shape `(batch, out_dims)` if `apply_pooling` else
                  `(batch, max_atoms, out_dims)`
            Keys are omitted if the corresponding capture flag is False.
        """
        if loader is None:
            loader = self.data_loader or self.train_loader
        assert loader is not None, 'No data loader available for embedding capture'
        ...
        try:
            with torch.no_grad():
                for data in loader:
                    X, _, _ = data
                    src, frac = X.squeeze(-1).chunk(2, dim=1)
                    src_dev = src.to(self.compute_device,
                                     dtype=torch.long,
                                     non_blocking=True)
                    frac_dev = frac.to(self.compute_device,
                                       dtype=data_type_torch,
                                       non_blocking=True)

                    encoded = None
                    if capture_encoder or capture_head:
                        encoded = self.model.encoder(src_dev, frac_dev)

                    if capture_encoder:
                        encoder_batches.append(encoded.detach().cpu())

                    if capture_head:
                        head_output = self.model.output_nn(encoded)
                        if apply_pooling:
                            mask = (src_dev == 0).unsqueeze(-1)
                            head_output = head_output.masked_fill(mask, 0.0)
                            counts = (~mask).sum(dim=1, keepdim=True).clamp_min(1)
                            head_output = head_output.sum(dim=1) / counts
                            head_output = head_output.reshape(head_output.size(0), -1)
                            if apply_gating and head_output.size(-1) > 1:
                                values, logits = head_output.chunk(2, dim=-1)
                                prob = torch.ones_like(values)
                                prob[:, :logits.size(-1)] = torch.sigmoid(logits)
                                head_logit_batches.append(logits.detach().cpu())
                                head_output = values * prob
                        head_batches.append(head_output.detach().cpu())
        finally:
            if was_training:
                self.model.train()

        if capture_encoder:
            captures['encoder'] = torch.cat(encoder_batches, dim=0)
        if capture_head:
            captures['head'] = torch.cat(head_batches, dim=0)
            if apply_pooling and apply_gating and head_logit_batches:
                captures['head_logits'] = torch.cat(head_logit_batches, dim=0)
